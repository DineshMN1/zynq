...
/**
 * Determine whether a filename's extension is eligible for duplicate detection.
 *
 * Only document and image extensions are considered eligible.
 *
 * @param fileName - The filename to inspect (may include an extension)
 * @returns `true` if the file's extension is in the duplicate-check list, `false` otherwise.
 */
