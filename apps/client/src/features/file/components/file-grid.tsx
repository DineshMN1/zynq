...
/**
 * Render a file browser that displays folders and files in either a responsive grid or a list view with persistent view preference.
 *
 * The component shows a loading indicator when `loading` is true, an empty-state card when `files` is empty, and otherwise renders folders first then regular files. Users can toggle between grid and list layouts; the chosen view is stored in localStorage under "zynq-view-mode".
 *
 * @param files - Array of file metadata to display.
 * @param loading - If true, shows a loading state instead of file content.
 * @param onOpenFolder - Called when a folder item is opened.
 * @param onDelete - Called when a file or folder delete action is triggered.
 * @param onShare - Called when a file or folder share action is triggered.
 * @param selectedIds - Optional set of selected file IDs used to render selection state.
 * @param onToggleSelect - Called to toggle selection for a given file.
 * @param onCardClick - Called when a file or folder card or row is clicked.
 * @returns A React element that presents the provided files and folders with selection and action handlers in either grid or list layout.
 */
